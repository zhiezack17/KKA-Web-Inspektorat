...
$allowedOrigins = array_values(array_filter(array_map('trim', explode(',', (string)($cfg['mobile_api_allowed_origins'] ?? '')))));
...
function api_response(int $code, bool $success, string $message = '', $data = null): void {
    ...
    header('Content-Type: application/json; charset=utf-8');
    header('Cache-Control: no-store, no-cache, must-revalidate, max-age=0');
    echo json_encode(['success' => $success, 'code' => $code, 'message' => $message, 'data' => $data], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
    exit;
}

function api_input(): array {
    $method = strtoupper((string)($_SERVER['REQUEST_METHOD'] ?? 'GET'));
    if ($method === 'GET') return is_array($_GET) ? $_GET : [];
    $contentLength = (int)($_SERVER['CONTENT_LENGTH'] ?? 0);
    if ($contentLength > 2 * 1024 * 1024) api_response(413, false, 'Ukuran request terlalu besar.');
    ...
    if (str_contains($contentType, 'application/x-www-form-urlencoded') || str_contains($contentType, 'multipart/form-data')) return is_array($_POST) ? $_POST : [];
...
function api_validate(array $data, array $rules): ?string {
    foreach ($rules as $field => $rule) {
        foreach (explode('|', $rule) as $part) {
            if ($part === 'required' && (!isset($data[$field]) || trim((string)$data[$field]) === '')) return "Kolom $field wajib diisi";
            if ($part === 'email' && isset($data[$field]) && !filter_var($data[$field], FILTER_VALIDATE_EMAIL)) return "Format email $field tidak valid";
            if (str_starts_with($part, 'min:')) { $min = (int)explode(':', $part, 2)[1]; if (isset($data[$field]) && strlen((string)$data[$field]) < $min) return "Kolom $field minimal $min karakter"; }
            if ($part === 'numeric' && isset($data[$field]) && !is_numeric($data[$field])) return "Kolom $field harus berupa angka";
        }
    }
    return null;
}
...
final class ApiAuth {
    private const DEFAULT_ACCESS_TOKEN_TTL = 1800;
    ...
    public function authenticate(): ?array {
        ...
        $this->user = ['id' => (int)$row['user_id'], 'nama' => $row['nama'], 'email' => $row['email'], 'role' => $row['role'], 'nip' => $row['nip'], 'jabatan' => $row['jabatan'], 'is_active' => true];
        return $this->user;
    }
    ...
    public function attempt(string $identifier, string $password, string $deviceName = 'mobile'): ?array {
        ...
        $u = DB::one(
            'SELECT * FROM kka_users 
             WHERE (LOWER(email) = ? OR LOWER(username) = ? OR REPLACE(nip, " ", "") = ? OR nip = ?) 
             LIMIT 1',
            [$lower, $lower, $cleanNip, $identifier]
        );
        ...
        return ['token' => $token, 'user' => ['id' => (int)$u['id'], 'nama' => $u['nama'], 'email' => $u['email'], 'role' => $u['role'], 'nip' => $u['nip'], 'jabatan' => $u['jabatan']], 'expires_at' => $this->tokenExpiresAt()];
    }
}
