<?php
partial('head', ['title' => 'Edit Berita Acara Pemeriksaan Kas - ' . e($row['no_bap'])]);
partial('sidebar');
?>
    ...
    <div class="page-head">
      <div>
        <h2 style="display:flex;align-items:center;gap:10px">
          ...
          Edit Berita Acara Pemeriksaan Kas (Opname Kas Desa)
        </h2>
        <p>Perbarui perhitungan fisik kas tunai, mutasi bank, dan rekonsiliasi BKU Kepenghuluan <?= e($row['desa_nama']) ?> TA <?= $row['tahun_anggaran'] ?>.</p>
      </div>
      <div style="display:flex;gap:10px">
        <a href="<?= url('print/opname-kas?id=' . $row['id']) ?>" target="_blank" class="btn btn-outline" style="color:#059669;border-color:#059669">
          <i class="fa-solid fa-print"></i> Cetak A4
        </a>
        <a href="<?= url('opname-kas?tahun=' . $row['tahun_anggaran'] . '&desa_id=' . $row['desa_id']) ?>" class="btn btn-outline">
          <i class="fa-solid fa-arrow-left"></i> Kembali
        </a>
      </div>
    </div>

    <form method="POST" action="<?= url('opname-kas/update') ?>" id="formOpname">
      <?= csrf_field() ?>
      ...
      <div class="card" style="padding:22px;margin-bottom:20px">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(280px, 1fr));gap:16px">
          <div>
            <label class="form-label" style="font-weight:700">Kepenghuluan (Desa)</label>
            <input type="text" class="form-control" disabled value="<?= e($row['desa_nama']) ?> (Kec. <?= e($row['kecamatan_nama']) ?>)">
          </div>

          <div>
            <label class="form-label" style="font-weight:700">Nomor Berita Acara (BAP) <span style="color:#dc2626">*</span></label>
            <input type="text" name="no_bap" class="form-control" required value="<?= e($row['no_bap']) ?>">
          </div>

          <div>
            <label class="form-label" style="font-weight:700">Tanggal Pemeriksaan Kas <span style="color:#dc2626">*</span></label>
            <input type="date" name="tgl_pemeriksaan" class="form-control" required value="<?= e($row['tgl_pemeriksaan']) ?>">
          </div>

          <div>
            <label class="form-label" style="font-weight:700">Waktu &amp; Tempat Pemeriksaan</label>
            <div style="display:grid;grid-template-columns:120px 1fr;gap:8px">
              <input type="text" name="waktu_pemeriksaan" class="form-control" value="<?= e($row['waktu_pemeriksaan']) ?>">
              <input type="text" name="tempat_pemeriksaan" class="form-control" value="<?= e($row['tempat_pemeriksaan']) ?>">
            </div>
          </div>
        </div>
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(280px, 1fr));gap:16px;margin-top:16px">
          <div>
            ...
            <input type="text" name="nama_bendahara" class="form-control" required value="<?= e($row['nama_bendahara']) ?>">
          </div>

          <div>
            <label class="form-label" style="font-weight:700">Nama Pj. Penghulu / Penghulu <span style="color:#dc2626">*</span></label>
            <input type="text" name="nama_kepala_desa" class="form-control" required value="<?= e($row['nama_kepala_desa']) ?>">
          </div>

          <div>
            <label class="form-label" style="font-weight:700">Nama Ketua Tim Pemeriksa APIP <span style="color:#dc2626">*</span></label>
            <input type="text" name="nama_ketua_tim" class="form-control" required value="<?= e($row['nama_ketua_tim']) ?>">
          </div>

          <div>
            <label class="form-label" style="font-weight:700">NIP Ketua Tim Pemeriksa</label>
            <input type="text" name="nip_ketua_tim" class="form-control" value="<?= e($row['nip_ketua_tim'] ?? '') ?>">
          </div>
        </div>
      </div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px">
                  ...
                  <tr style="border-bottom:1px solid #f1f5f9">
                    ...
                    <td style="padding:6px 4px;text-align:center">
                      <input type="number" min="0" name="kertas_<?= $p ?>" class="form-control count-kertas" 
                             data-nominal="<?= $p ?>" value="<?= $qty ?>" style="width:85px;padding:4px 6px;text-align:center;font-weight:700;margin:0 auto" oninput="hitungTotalKas()">
                    </td>
                    <td style="padding:6px 4px;text-align:right;font-weight:700" id="sub_kertas_<?= $p ?>"><?= rupiah($sub) ?></td>
                  </tr>
                <?php endforeach; ?>
              </tbody>
              <tfoot>
                <tr style="border-top:2px solid #cbd5e1;font-weight:800;background:#f1f5f9">
                  ...
                  <td style="padding:8px 4px;text-align:right;color:#15803d;font-size:13px" id="total_kertas_view"><?= rupiah($row['total_kertas']) ?></td>
                </tr>
              </tfoot>
            </table>
          </div>
          ...
          <div style="background:#f8fafc;padding:16px;border-radius:8px;border:1px solid #e2e8f0">
              ...
              <tbody>
                ...
                ?>
                  <tr style="border-bottom:1px solid #f1f5f9">
                    ...
                    <td style="padding:6px 4px;text-align:right;font-weight:700" id="sub_logam_<?= $p ?>"><?= rupiah($sub) ?></td>
                  </tr>
                <?php endforeach; ?>
              </tbody>
              <tfoot>
                <tr style="border-top:2px solid #cbd5e1;font-weight:800;background:#f1f5f9">
                  ...
                  <td style="padding:8px 4px;text-align:right;color:#d97706;font-size:13px" id="total_logam_view"><?= rupiah($row['total_logam']) ?></td>
                </tr>
              </tfoot>
            </table>
            ...
            <div style="margin-top:20px;padding:14px;background:#e0f2fe;border:1.5px solid #7dd3fc;border-radius:8px">
              ...
              <div style="font-size:20px;font-weight:800;color:#0284c7;margin-top:2px" id="total_kas_fisik_view"><?= rupiah($row['total_kas_fisik']) ?></div>
      ...
      <div class="card" style="padding:22px;margin-bottom:20px">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(280px, 1fr));gap:16px">
          <div>
            ...
            <input type="text" name="nama_bank" class="form-control" value="<?= e($row['nama_bank'] ?? 'Bank Riau Kepri Syariah') ?>">
          </div>

          <div>
            <label class="form-label" style="font-weight:700">Nomor Rekening Giro Kas Desa</label>
            <input type="text" name="no_rekening_bank" class="form-control" value="<?= e($row['no_rekening_bank'] ?? '') ?>" placeholder="Contoh: 104-08-01234-5">
          </div>
          ...
          <div>
            ...
            <div style="position:relative">
              ...
              <input type="text" name="saldo_bank" id="saldo_bank" class="form-control" style="padding-left:36px;font-weight:700;font-size:14px;color:#6d28d9" value="<?= number_format((float)$row['saldo_bank'], 0, ',', '.') ?>" oninput="formatAndCalc(this)">
            </div>
          ...
          <div>
            ...
            <div style="position:relative">
              <span style="position:absolute;left:10px;top:8px;font-weight:700;color:var(--slate-500);font-size:13px">Rp</span>
              <input type="text" name="saldo_bku" id="saldo_bku" class="form-control" style="padding-left:36px;font-weight:700;font-size:14px;color:#0f172a" value="<?= number_format((float)$row['saldo_bku'], 0, ',', '.') ?>" oninput="formatAndCalc(this)">
            </div>
        ...
        <div style="margin-top:22px;padding:18px 20px;border-radius:8px;background:#f8fafc;border:1.5px solid #cbd5e1" id="boxRekonsiliasi">
          <div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(200px, 1fr));gap:14px;align-items:center">
            <div>
              ...
              <div style="font-size:18px;font-weight:800;color:#0369a1;margin-top:2px" id="total_kas_riil_view"><?= rupiah($row['total_kas_riil']) ?></div>
            ...
            <div>
              ...
              <div style="font-size:18px;font-weight:800;color:#0f172a;margin-top:2px" id="saldo_bku_view"><?= rupiah($row['saldo_bku']) ?></div>
            ...
            <div>
              ...
              <div style="font-size:20px;font-weight:800;color:#16a34a;margin-top:2px" id="selisih_kas_view"><?= rupiah($row['selisih_kas']) ?></div>
        ...
        <div style="margin-top:16px">
          ...
          <textarea name="penjelasan_selisih" class="form-control" rows="2" placeholder="Jelaskan penyebab selisih kas fisik vs BKU..."><?= e($row['penjelasan_selisih'] ?? '') ?></textarea>
        </div>

        <div style="margin-top:14px">
          <label class="form-label" style="font-weight:700">Catatan &amp; Arahan Tim Pemeriksa APIP</label>
          <textarea name="catatan_pemeriksaan" class="form-control" rows="2" placeholder="Catatan integritas penyimpanan brankas..."><?= e($row['catatan_pemeriksaan'] ?? '') ?></textarea>
        </div>
      </div>

      <!-- ACTION BUTTONS -->
      <div style="display:flex;justify-content:flex-end;gap:12px;margin-bottom:40px">
        <a href="<?= url('opname-kas?tahun=' . $row['tahun_anggaran'] . '&desa_id=' . $row['desa_id']) ?>" class="btn btn-outline">Batal</a>
        <button type="submit" class="btn btn-primary" style="background:#059669;border-color:#059669;padding:10px 24px;font-size:14px;font-weight:700">
          <i class="fa-solid fa-floppy-disk"></i> Simpan Perubahan BAP Kas
        </button>
      </div>
